...
program
  .addOption(
    program
      .createOption("--url <url>", "The URL of the resource to fetch")
  ...
  .addOption(
    program
      ...
      .makeOptionMandatory(false)
      .env("FX_PREVIEW_TRIGGER")
      .default("DELAY"),
  )
  .addOption(
    program
      .createOption("--delay <delay>", "The delay before the capture is taken")
      .makeOptionMandatory(false)
      .env("FX_PREVIEW_DELAY")
      .default("10"),
  )
  .addOption(
    program
  ...
  .addOption(
    program
      ...
      .makeOptionMandatory(false)
      .env("FX_PREVIEW_SELECTOR"),
  )
  .addOption(
    program
      .createOption("--output <dir>", "The location to output the capture to")
      .makeOptionMandatory(false)
      .env("FX_PREVIEW_OUTPUT")
      .default("./output"),
  );
...
const main = async () => {
  ...
  try {
    ...
    try {
      ...
      const newLocal = path.join(output, "preview.png");
      fs.writeFileSync(newLocal, capture);
    } catch (error) {
      console.error(error);
    }
    return;
